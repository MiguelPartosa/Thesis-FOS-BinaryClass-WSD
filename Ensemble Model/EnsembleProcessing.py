...
def process_embeddings(df, variance_threshold=0.8, embedding_model=None):
    """
    ...
    df_pca_sentence = generate_embedding_df('Sentence Embeddings', df_embedded['Sentence Embeddings'])
    df_pca_verb = generate_embedding_df('Verb Embeddings', df_embedded['Verb Embeddings'])
    df_pca_usage = generate_embedding_df('Usage Embeddings', df_embedded['Usage Embeddings'])
    
    # Find optimal number of components
    scaler = StandardScaler()
    verb_components = get_components_for_variance(df_pca_verb, variance_threshold, scaler)
    usage_components = get_components_for_variance(df_pca_usage, variance_threshold, scaler)
    sentence_components = get_components_for_variance(df_pca_sentence, variance_threshold, scaler)
    ...
    sentence_k = find_optimal_clusters(df_pca_sentence, sentence_components, scaler)
    ...
    final_pca_verb = generate_guided_pca(df_pca_verb, verb_components, verb_k, 'Verb', scaler)
    final_pca_usage = generate_guided_pca(df_pca_usage, usage_components, usage_k, 'Usage', scaler)
    final_pca_sentence = generate_guided_pca(df_pca_sentence, sentence_components, sentence_k, 'Sentence', scaler)
    
    # Combine results
    final_result = pd.concat([final_pca_usage, final_pca_verb, final_pca_sentence], axis=1)
...
def generate_embedding_df(col_name, embeddings_series):
    ...
    for embeddings in tqdm(embeddings_series, desc=f'Transforming {col_name}'):
        ...
        df = pd.concat([df, pd.DataFrame([new_row_val], columns=new_row_column_name)])
...
def generate_guided_pca(df, components, clusters, title, scaler):
    ...
    # Apply KMeans
    kmeans_model = KMeans(n_clusters=clusters, init="k-means++", random_state=42)
